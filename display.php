...
$nom = $_SESSION['nom'];
$sql = "select paid from tripd where tname='$tname' AND uid='$uname'";
$result = $conn->query($sql);
$amt=0;
//$total = mysqli_fetch_array($result,MYSQLI_ASSOC);
while($total = mysqli_fetch_array($result,MYSQLI_ASSOC))    
    {
       $amt = $amt + $total['paid'];  
    }
    if(isset($amt))
...
$sql1 = "select mem from tripd where tname='$tname' AND uid='$uname'";
...
<div class="container">
 ...
 <?php
   ...
   // if($_SESSION['invalid']==1)
    ...
    ?>
    <div class="container">
  <div class="row " style="color: white; font-family: "Times New Roman", Times, serif;">
   <div class=""> 
       <table  class="table table-hover  table-condensed text-center" style=" background-color: darkorange; border-width: 5px; border-color: maroon; border: 2px; ">
    <thead style="border-collapse:seperate;">
        <tr>
            <td><h4><b>-------------- Trip Summary ---------------</b></h4></td>
      </tr>
    </thead>
    ...
    while($values = mysqli_fetch_array($result1,MYSQLI_ASSOC) and $test = mysqli_fetch_array($result,MYSQLI_ASSOC))    
    {
        if($avg > $test['paid'])
        {
            $pay = $avg-$test['paid'];
        ?>
            ...
            <td>
                <h4>
                    <?php echo $values['mem'];  ?>  Need to pay <?php echo $pay;  ?> more
                </h4>
            </td>
        </tr>
        <?php
        }
        else if($avg < $test['paid']) 
        {
         $get = $test['paid']-$avg;
         ?>  
        <tr class="info">
            <td>
                <h4>
                    <?php echo $values['mem'];  ?> Need to receive <?php echo $get;  ?>
                </h4>
            </td>
        </tr>
        ...
        <tr class="info">
            <td>
                <h4>
                    <?php echo $values['mem'];  ?> Need not have to pay anything
                </h4>
            </td>
        </tr>
        <?php
  ...
  <div class="container">  
<table  class="table table-hover  table-condensed text-center" style=" background-color: white; border-width: 5px; border-color: maroon; border: 2px; ">
    <tr>
        <td>
